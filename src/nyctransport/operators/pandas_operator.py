...
class PandasOperator(BaseOperator):
    ...
    def __init__(self,
        ...
        self._input_callable_kwargs = output_callable_kwargs or {}

        # Attributes for transformations
        self._transformation_callable = transform_callable
        self._transformation_callable_kwargs = transform_callable_kwargs or {}
    ...
    def execute(self, context):
        ...
        logging.info("Reading DataFrame with shape: %s", df.shape)

        if self._transformation_callable:
            df = self._transformation_callable(df, **self._transformation_callable_kwargs)
