...
from airflow.src.nyctransport.operators.pandas_operator import PandasOperator
from airflow.src.nyctransport.operators.s3_to_postgres import MinioPandasToPostgres
...
def _download_taxi_data():
    ...
    for filename in [f["name"] for f in files]:
        ...
        try:
            s3_hook.load_string(
                string_data=response.text, key=s3_key, bucket_name="data_lake"
        )
...
def transform_citi_biki_data(df):
    # Map citi bike lat, lon coordinates to taxi zone ids
    taxi_zones = geopandas.read_file(
        "https://s3.amazoneaws.com/nyc-tlc/misc/taxi_zones.zip"
    ).to_crs("EPSG:4326")
...
process_citi_bike_data = PandasOperator(
    ...
    transform_callable=transform_citi_biki_data,
...
proces_taxi_data =PandasOperator(
    ...
    output_callable_kwargs={
        "bucket": "datalake",
        "path": "processed/taxis/{{ ts_nodash }}.parquet",
        "pandas_write_callable": pd.DataFrame.to_parquet,
        "pandas_write_callable_kwargs": {"engine": "auto"},
    },
    dag=dag,
...
download_taxi_data >> proces_taxi_data >> taxi_to_db