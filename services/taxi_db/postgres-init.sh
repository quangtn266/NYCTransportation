...
for url in ${urls}
do
  time wget "${url}" -0 /tmp/yellowtripdata.csv
  # Importing all records results in a 6.35GB image
  # Therefore, we select every 10th line to decrease size and end up with a 1.21 GB Docker image.
  time awk -F',' 'NR == 1 || NR % 10 == 0 {print $2","$3","$5","$8","$9}' /tmp/yellowtripdata_small.csv
  time psql -v ON_ERROR_STOP=1 tcltriprecords <<-EOSQL
    COPY triprecords(pickup_datetime,dropoff_datetime,trip_distance,pickup_locationid,dropoff_locationid)
    FROM '/tmp/yellowtripdata_small.csv' DELIMETER ',' CSV HEADER;
EOSQL
done

psql -v ON_ERROR_STOP=1 <<-EOSQL
  CREATE USER taxi WITH PASSWORD 'ridetcl';
