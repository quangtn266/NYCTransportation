...
psql -v ON_ERROR_STOP=1 citibike <<-EOSQL
  CREATE TABLE IF NOT EXIST tripdate (
...
for url in ${urls}
do
  wget "${url}" -0 /tmp/citibike-tripdata.csv.zip # Download data
  unzip /tmp/citibike-tripdata.csv.zip -d /tmp # Unzip
  filename=$(echo ${url} | sed 's:.*/::' | sed 's/\.zip$//') # Determine filename of unzipped CSV (this is the same as .zip file)
  # Filter lines otherwise full docker image is 4.18 Gib, every 8th line result in 890Mb
